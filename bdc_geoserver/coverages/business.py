...
class CoverageBusiness():
    ...
    def publish(cls, data):
        datastore = replace_by_list(
            data['datastore'], ['.', '-', '_', '.', '~', '@', '!', '/'], '')
        layer = replace_by_list(
            data['layer'], ['.', '-', '_', '.', '~', '@', '!', '/'], '')
        ...
        datacube_path = Path(
            '{}/{}/'.format(os.environ.get('PATH_CUBES_FILE'), layer))
        for filename in os.listdir(properties_path):
            shutil.copy2('{}/{}'.format(properties_path, filename),
                         '{}/{}'.format(datacube_path, filename))

        """
            Create coverageStore
        """
        path = '{}{}'.format(os.environ.get(
            'PATH_BASE_GEOSERVER'), data['path'])
        result_create = CoverageServices.create_coveragestore(
            data['workspace'], datastore, layer, path)
        ...
        </coverage>".format(layer, layer, layer, data['description'],
                            data['projection'].replace(" ", ""))

        result_publish = CoverageServices.publish_layer(
            data['workspace'], datastore, layer, bodyXML)
    ...
    def unpublish(cls, workspace, datastore, layer):
        ...
        CoverageServices.unpublish(workspace, layer)
        CoverageServices.remove(workspace, datastore, layer)
        ...
        db.engine.execute('DROP TABLE IF EXISTS "{}{}"'.format(
            layer, os.environ.get('SUFFIX_NAME_CUBE', '_MEDIAN')))
        return True
