...
class CoverageServices():
    ...
    def create_coveragestore(cls, workspace, datastore, layer, path):
        ...
        base_url += '/rest/workspaces/{}/coveragestores/{}/external.imagemosaic?configure=all&coverageName={}'.format(
            workspace, datastore, layer)

        r = requests.put(base_url, headers=headers,
                         data=path, verify=False, auth=auth)
        if r and r.status_code in (200, 201):
            return r
        return None
    ...
    def publish_layer(cls, workspace, datastore, layer, bodyXML):
        ...
        base_url += '/rest/workspaces/{}/coveragestores/{}/coverages/{}'.format(
            workspace, datastore, layer)
        body = re.sub(r'/(\r\n\t|\n|\r\t)/gm', '', bodyXML)

        r = requests.put(base_url, headers=headers,
                         data=body, verify=False, auth=auth)
        if r and r.status_code in (200, 201):
            return r
        return None
    ...
    def remove(cls, workspace, coverage_store, layer):
        base_url, headers, auth = cls.get_base_url(content_type='text/plain')
        base_url += '/rest/workspaces/{}/coveragestores/{}?recurse=true'.format(
            workspace, coverage_store)
